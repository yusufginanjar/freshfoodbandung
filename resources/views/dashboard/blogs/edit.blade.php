...
<div class="col-lg-8">
    ...
    <form method="POST" action="/{{ config('app.path') }}/dashboard/blogs/{{ $blog->slug }}" class="mb-5" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="mb-3">
            <label for="title" class="form-label">Edit Blog</label>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" required value="{{ old('slug', $blog->slug) }}" placeholder="title-number-one">
            @error('slug')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="mb-3">
            ...
            @if ( $blog->image )
                <img src="{{ asset('storage/' . $blog->image) }}" alt="" class="img-preview img-fluid mb-3 col-sm-5 col-lg-3 d-block">
            @else
